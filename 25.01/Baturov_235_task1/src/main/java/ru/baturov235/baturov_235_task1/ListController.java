    ...
    void addButOnAction(ActionEvent event) {
        try {
            ...
            if (number >= 0 && number <= 30000) {
                View.getItems().add(number);
                counter++;
                if (counter > 1000) {
                    answerLabel.setText("Превышено максимально допустимое количество чисел");
                }
            } else {
                answerLabel.setText("Число вне диапазона от 0 до 30000");
            }
            TextField.clear();
        } catch (NumberFormatException ex) {
            answerLabel.setText("Введите число");
        }
